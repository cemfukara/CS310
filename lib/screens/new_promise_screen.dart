...
class _NewPromiseScreenState extends State<NewPromiseScreen> {
  ...
  Future<void> _handleCreatePromise() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> createdSlotData = [];
      ...
      for (var slot in dynamicSlots) {
        if (slot['start'] != null) {
          ...
          final id = await promiseProvider.addPromise(
          ...
          createdPromiseIds.add(id);
          createdSlotData.add({
            'start': slot['start'],
            'end': slot['start'].add(Duration(minutes: totalMinutes)),
          });
        }
      }

      // 2. SEND LINKED FRIEND REQUESTS
      if (_selectedFriendUids.isNotEmpty) {
        await _sendLinkedPromiseRequests(createdPromiseIds, createdSlotData);
      }
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        for (var friendUid in _selectedFriendUids) {
          // For each created promise
          for (int i = 0; i < promiseIds.length; i++) {
            final pid = promiseIds[i];
            final data = slotData[i];
            ...
            final request = PromiseRequestModel(
              ...
              priority: difficultyStars,
              sentAt: DateTime.now(),
              linkedPromiseId: pid, // LINK TO EXISTING DOC
            );
